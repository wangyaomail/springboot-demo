...
import demo.common.bean.job.QueueJob;
import demo.common.service.BaseService;
import demo.common.util.time.TimeUtil;
...
public class QueueServer extends BaseService {
    ...
    public void addAJob(QueueJob job) {
        if (sysConfService.getSwitchOnlineQueue()) {
            try {
                jobQueue.put(job);
    ...
    public void start() {
        if (sysConfService.getSwitchOnlineQueue()) {
            logger.info("queue start parsing. thread size="
                    + sysConfService.getQueueSlaveSize());
    ...
    public boolean isQueueEnable() {
        return sysConfService.getSwitchOnlineQueue();
    }
}
